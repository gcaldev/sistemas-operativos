...
{  // K
	// verifico si la prioridad es entre 0 a 100
	...
	// verifico que la prioridad a asignar sea menor que la actual
	// 0 es tope, 100 es baja
	if (priority < env->env_priority) {
		return -E_INVAL;
	}
...
int
sys_env_get_priority(envid_t envid, int *dest)
{  // K
	struct Env *env;
	int r;
	if ((r = envid2env(envid, &env, 1)))
...
bail:
	...
	// Desbloquear proceso y configurar prioridad simil proceso recien creado
	dstenv->env_priority = 0;
	return 0;
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_exofork:
		return sys_exofork();
	case SYS_env_set_priority:  // K
		return sys_env_set_priority(a1, a2);
	case SYS_env_get_priority:                            // K
		return sys_env_get_priority(a1, (int *) a2);  // TODO: placeholder
