...
fork_v0(void)
{
	...
	int r;
	if ((r = sys_env_get_priority(envid, &priority)) < 0)  // K
		panic("sys_env_get_priority: %e", r);

	if (priority < 100 &&
	    (r = sys_env_set_priority(envid, priority + 1)) < 0)  // K
...
fork(void)
{
	...
	if ((r = sys_env_get_priority(envs[ENVX(sys_getenvid())].env_id,
	                              &priority)) < 0)  // K
		panic("sys_env_get_priority: %e", r);
